...
export class HomeComponent implements OnInit {
	...
	reviews = [
		{
			id: 0,
			user: 'Strahinja ',
			aboutUser: '6 godina, Beograd',
			review: 'Otežana komunikacija, dispazični govor. Posle uzimanja ½ kapsule Linčži dete je potpuno tečno progovorilo I normalno komunicira. Preparate kompanije FOHOW nam je preporučila naša saradnica pedijatar. Roditelji su u šoku.'
		},
		{
			id: 1,
			user: 'Borislav',
			aboutUser: 'Novi Sad',
			review: 'Infekcija oka kod deteta. Mesecima je bezuspešno lečio hronični konjuktivitis. Posle ukapavanja eliksira Kordiceps Fohow u oko za samo dva dana infekcija je sanirana.'
		},
		{
			id: 2,
			user: 'Dragana T.',
			aboutUser: 'Novi Sad',
			review: 'Imala sam vodenu cistu na levom jajniku. Pila sam hormonalnu terapiju 3 meseca i nije mi pomogla. Iskoristila sam 5 tampona. Na pregledu je utvrđeno da je levi jajnik zdrav i da ciste više nema.'
		},
		{
			id: 3,
			user: 'Irina',
			aboutUser: '36 godina, Ljig',
			review: 'Koristila sam tampon Guifei Bao, savršeni su. Posle porođaja imala sam gljivičnu infekciju. Nisu mi pomogle nikakve supozitorije, imala sam jak svrab i peckanje. Stavljala sam tampone i posle 4 tampona sve je nestalo.'
		},
		{
			id: 4,
			user: 'J. Matić',
			aboutUser: null,
			review: 'Imala sam česte upale jajnika, bolove pred ciklus koji su bili nesnosni. Iskoristila sam svih 6 tampona, osećam se sjajno, neverovatno su moćni, osećam se lakšom, bolovi su nestali, na ginekološkom pregledu od upale jajnika ni traga. Fenomenalno, preporučila bi svakoj ženi tampone Biserna princeza.'
		},
		{
			id: 5,
			user: 'Irina ',
			aboutUser: '41 godina',
			review: 'Šestog dana posle uzimanja čaja Ljuvej Fohow i voćne paste Roza Fohow nestalo je zadebljanje mlečne žlezde i podmišićni limfni čvorovi. Koristila sam 3 dana faradoterapeutski pojas i bolovi kičme u oblasti trtične kosti su nestali.'
		}
	];
	...
	currentReview: any;
 	...
 	onChangeReview(isNext) {
 		if (isNext) {
 			if (this.currentReview.id === (this.reviews.length - 1)) {
 				this.currentReview = this.reviews[0];
 			} else {
 				this.currentReview = this.reviews[this.currentReview.id + 1];
 			}
 		} else {
 			if (this.currentReview.id === 0) {
 				this.currentReview = this.reviews[this.reviews.length - 1];
 			} else {
 				this.currentReview = this.reviews[this.currentReview.id - 1];
 			}
 		}
